...
> Try to keep to suggested timings below

## AI Use Policy for Week 2

:::{.callout-important collapse=true}

## AI Tools (ChatGPT, Claude, GitHub Copilot, etc.) Are NOT *recommended* in Week 3
...
Week Three is about developing your mental model of data modelling:

    - Try and write code for yourself
    
    - IF you are completely stuck try using Copilot
    
    - Evaluate code produce against examples from the coursebook - try to match style and evaluate your understanding of what it is doing.
:::
...
8. Write a clear commit message "Data cleaning and validation [include some details on what issues were documented and fixed]" and push to Github
...
# Read data ----
parasites_raw <- read_csv(here("data", "parasite_coinfection.csv"))
...
- At each stage **try and write yourself comments on the models and code** - discuss with others

1. **Start simple**: Fit a linear model
   - Model: `lm(trematode_count ~ fish_length_mm + infection_status + fish_length_mm:infection_status)`
   - Examine diagnostics—what problems do you see?

2. **Fit Poisson GLM** (additive model first)
   - Model: `glm(trematode_count ~ fish_length_mm + infection_status, family = poisson)`
...
# read_csv(here("data", "clean_parasite_coinfection.csv"))
