...
## Write your PAT somewhere safe and use this at the password prompt
...
- If stored in a file, add this file name to your `.gitignore` file
...
- You now need to use R commands when making commits and pushes to Github 
...
# Commit
gert::git_commit("Your commit message")
...
# Load data
mosquito_egg_raw = pd.read_csv("data/mosquito_egg_data.csv")
mosquito_egg_raw.columns = mosquito_egg_raw.columns.str.lower()

# Basic overview
mosquito_egg_raw.info()
mosquito_egg_raw.describe(include="all")
mosquito_egg_raw.info()

# Interactive table
# view() is an RStudio-specific spreadsheet viewer; in Jupyter/VS Code,
# display the frame directly or use the editor's own variable viewer
mosquito_egg_raw

# Counts by site and treatment
mosquito_egg_raw.groupby(["site", "treatment"], as_index=False).size()
...
- Stage both `mosquito_egg_data.csv` and `initial_exploration.R`
