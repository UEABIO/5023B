# Logistic regression (for binary data) {#sec-logistic}
...
beetles.columns = beetles.columns.str.lower()
beetles = beetles.assign(number_survived=lambda d: d["number_tested"] - d["number_killed"])
```
...
```{python}
#| eval: false
# TRANSLATION-NOTE: OQ-033
binomial_model = smf.glm(
    "number_killed + number_survived ~ dose", data=beetles, family=sm.families.Binomial()
).fit()
```
...
```{python}
#| eval: false
# TRANSLATION-NOTE: OQ-036
grid = pd.DataFrame({"dose": np.arange(49, 78)})
grid.assign(**binomial_model.get_prediction(grid).summary_frame())
```
...
```{python}
#| eval: false
grid = pd.DataFrame({"dose": np.arange(40, 81)})
predictions = grid.assign(**binomial_model.get_prediction(grid).summary_frame())

(
    ggplot(predictions, aes("dose", "mean"))
    + geom_point(data=beetles, mapping=aes(y="mortality_rate"))
    + geom_line(aes(y="mean"))
    + geom_ribbon(aes(ymin="mean_ci_lower", ymax="mean_ci_upper"), alpha=0.2)
)
```
...
Mortality rate increased with pesticide dose in the red flour beetle (*Tribolium castaneum*). Across the observed pesticide dosage each mg of pesticide was associated with a .25 [95%CI: 0.21 - 0.29] increase in the log odds of mortality (z = 11.42, P < 0.001). At the lowest pesticide dose (49mg) we observed a mean mortality of 7% [4.4 - 11.8%], at the middle range (60mg) we observed a mean mortality of 53.9% [47.4 - 90%] and at the highest dosage (77mg) we saw almost total mortality 98.7%[97.2 - 99.4%].
...
Binomial model shows better explanatory power (R-squared) for our correctly fitted model:
...
1 - binomial_model.llf / binomial_model.llnull
```
