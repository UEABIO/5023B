...
```{python}
#| eval: false
# TRANSLATION-NOTE: OQ-062
dogs_raw = pd.read_csv("data/dogs_imputed_reduced.tsv", sep=r"\s+")
dogs_raw.columns = dogs_raw.columns.str.lower()
...
(
    ...
    .filter(like="chr")
    .iloc[:, :9]
    .melt(var_name="snp", value_name="genotype")
    .value_counts("genotype")
    .reset_index(name="n")
)
```
...
(
    dogs_raw
    .assign(phenotype_label=lambda d: np.where(d["phenotype"] == 2, "Affected", "Unaffected"))
    .pipe(lambda d: (
        ggplot(d, aes("phenotype_label", fill="phenotype_label"))
        + geom_bar(alpha=0.85, show_legend=False)
        + scale_fill_manual(values={"Affected": "#E63946", "Unaffected": "#457B9D"})
        + labs(title="Phenotype distribution", x=None, y="Count")
        + theme_minimal()
    ))
)

# ggsave("outputs/phenotype_distribution.png", width=5, height=4)
```
...
(
    ...
    .filter(like="chr")
    .melt(var_name="snp", value_name="genotype")
    .assign(genotype=lambda d: d["genotype"].astype(str))
    .pipe(lambda d: (
        ggplot(d, aes("genotype", fill="genotype"))
        + geom_bar(show_legend=False)
        + facet_wrap("snp", scales="free_y")
        + scale_fill_manual(values={"0": "#457B9D", "1": "#2a9d8f", "2": "#E63946"})
        + labs(
            title="Genotype frequency distributions across a sample of SNP sites",
            x="Genotype (0, 1, or 2 copies of reference allele)",
            y="Count",
...
# ggsave("outputs/genotype_distributions.png", width=12, height=8)
```
...
# Save for use in 02_model.R
# We save the raw cleaned version — recoding happens in the modelling script
dogs_raw.to_csv("data/cleft_lip_clean.csv", index=False)
```
...
(
    ...
    .pipe(lambda d: (
        ...
        + facet_wrap("snp", scales="free_y")
        + scale_fill_manual(values={"0": "#457B9D", "1": "#2a9d8f", "2": "#E63946"})
        + labs(title="Genotype frequency distributions", x="Genotype", y="Count")
        + theme_minimal()
    ))
)
...
#| eval: false
# Load the cleaned dataset produced by 01_explore.R
dogs = pd.read_csv("data/cleft_lip_clean.csv")
```
...
dogs_glm = (
    dogs
    .drop(columns=["fid", "iid", "pat", "mat", "sex"])
    .assign(phenotype=lambda d: np.where(d["phenotype"] == 2, "affected", "unaffected"))
)

# TRANSLATION-NOTE: OQ-058
# Fit a logistic regression the familiar way using one SNP predictor
# This is the inferential framing you have used before
formula = "phenotype ~ " + " + ".join(c for c in dogs_glm.columns if c != "phenotype")
glm_base = smf.glm(formula, data=dogs_glm, family=sm.families.Binomial()).fit()
...
# For a logistic regression the most relevant panels are
# binned residuals and influential observations
sm.qqplot(glm_base.resid_deviance, line="45")
plt.scatter(glm_base.fittedvalues, glm_base.resid_deviance)
```
...
# Confirm the recoding and class balance
dogs_model["phenotype"].value_counts()
```
...
dog_train, dog_test = train_test_split(
    dogs_model, train_size=0.8, stratify=dogs_model["phenotype"], random_state=42
)

# Confirm class balance is maintained
dog_train["phenotype"].value_counts()
dog_test["phenotype"].value_counts()
```
...
#| eval: false
# Split 1: baseline — 80/20, seed 42
# random_state ensures the same dogs land in training and test
# every time this script is run — without it results change each run

# train_test_split() divides the data into training and test sets
# train_size = 0.8 means 80% goes to training, 20% to the test set
# stratify = dogs_model["phenotype"] ensures affected and unaffected dogs
# appear in similar proportions in both sets — essential when imbalanced
train_1, test_1 = train_test_split(
    dogs_model, train_size=0.8, stratify=dogs_model["phenotype"], random_state=42
)

# Pipeline: normalise predictors, then fit logistic regression
# StandardScaler rescales SNP predictors to mean 0, standard deviation 1
fit_1 = Pipeline([
    ("normalize", StandardScaler()),
    ("model", LogisticRegression()),
]).fit(train_1.drop(columns="phenotype"), train_1["phenotype"])

# Evaluate on the held-out test set
# predict() generates class labels using the default 0.5 threshold
# accuracy_score() calculates the proportion of dogs correctly classified
acc_1 = accuracy_score(test_1["phenotype"], fit_1.predict(test_1.drop(columns="phenotype")))
...
train_2, test_2 = train_test_split(
    dogs_model, train_size=0.7, stratify=dogs_model["phenotype"], random_state=42
)

fit_2 = Pipeline([
    ("normalize", StandardScaler()),
    ("model", LogisticRegression()),
]).fit(train_2.drop(columns="phenotype"), train_2["phenotype"])

acc_2 = accuracy_score(test_2["phenotype"], fit_2.predict(test_2.drop(columns="phenotype")))
...
train_3, test_3 = train_test_split(
    dogs_model, train_size=0.8, stratify=dogs_model["phenotype"], random_state=123
)

fit_3 = Pipeline([
    ("normalize", StandardScaler()),
    ("model", LogisticRegression()),
]).fit(train_3.drop(columns="phenotype"), train_3["phenotype"])

acc_3 = accuracy_score(test_3["phenotype"], fit_3.predict(test_3.drop(columns="phenotype")))
...
```{python}
#| eval: false
# step_naomit() drops missing genotype values before fitting; done here on
# the training frame directly since Pipeline has no row-dropping step
dog_train = dog_train.dropna()

# Pipeline: normalise predictors, then fit logistic regression
logistic_workflow = Pipeline([
    ("normalize", StandardScaler()),
    ("model", LogisticRegression()),
])

# Fit on training data only
logistic_fit = logistic_workflow.fit(dog_train.drop(columns="phenotype"), dog_train["phenotype"])
```
...
# manually — scikit-learn has no check_model()-style diagnostic panel
# This runs on training data — it checks model fit, not generalisation
train_probs = logistic_fit.predict_proba(dog_train.drop(columns="phenotype"))[:, 1]
train_resid = (dog_train["phenotype"] == "affected").astype(int) - train_probs

plt.scatter(train_probs, train_resid)
```
...
# TRANSLATION-NOTE: OQ-062
# Generate both class probabilities and predicted class labels
X_test = dog_test.drop(columns="phenotype")
pred_probs = logistic_fit.predict_proba(X_test)
logistic_preds = dog_test[["phenotype"]].assign(
    pred_affected=pred_probs[:, list(logistic_fit.classes_).index("affected")],
    pred_unaffected=pred_probs[:, list(logistic_fit.classes_).index("unaffected")],
    pred_class=logistic_fit.predict(X_test),
)
...
```{python}
#| eval: false
confusion_matrix(logistic_preds["phenotype"], logistic_preds["pred_class"], labels=["affected", "unaffected"])
```
...
#| eval: false
# TRANSLATION-NOTE: OQ-062
cm = confusion_matrix(logistic_preds["phenotype"], logistic_preds["pred_class"], labels=["affected", "unaffected"])
cm_df = (
    pd.DataFrame(cm, index=["affected", "unaffected"], columns=["affected", "unaffected"])
    .rename_axis(index="truth", columns="prediction")
    .reset_index()
    .melt(id_vars="truth", var_name="prediction", value_name="n")
)

(
    ggplot(cm_df, aes("prediction", "truth", fill="n"))
    + geom_tile()
...
pd.DataFrame({
    ...
    "estimate": [
        recall_score(logistic_preds["phenotype"], logistic_preds["pred_class"], pos_label="affected"),
        recall_score(logistic_preds["phenotype"], logistic_preds["pred_class"], pos_label="unaffected"),
        accuracy_score(logistic_preds["phenotype"], logistic_preds["pred_class"]),
    ],
})
```
...
#| eval: false
# TRANSLATION-NOTE: OQ-062
fpr, tpr, _ = roc_curve(logistic_preds["phenotype"] == "affected", logistic_preds["pred_affected"])
...
logistic_preds = logistic_preds.assign(
    pred_class=lambda d: np.where(d["pred_affected"] > chosen_threshold, "affected", "unaffected")
)
...
#| eval: false
(
    ggplot(logistic_preds, aes("pred_affected", fill="phenotype"))
    + geom_histogram(bins=20, alpha=0.7, position="identity")
    + scale_fill_manual(values={"affected": "#E63946", "unaffected": "#457B9D"})
    + geom_vline(xintercept=chosen_threshold, linetype="dashed", colour="grey30")
    + labs(
...
def tidy_coefs(pipeline, feature_names):
    ...
    return pd.DataFrame({
        ...
        "estimate": [model.intercept_[0]] + list(model.coef_[0]),
    })

feature_names = dog_train.drop(columns="phenotype").columns
logistic_coefficients = tidy_coefs(logistic_fit, feature_names)
...
(
    ...
    .assign(estimate=lambda d: np.exp(d["estimate"]))
