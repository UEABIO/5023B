...
```{python}
#| eval: false
bats_raw = pd.read_csv("data/bat_methylation.csv")
bats_raw.columns = bats_raw.columns.str.lower()
```
...
#| eval: false
(
    ggplot(bats_raw, aes("age"))
    + geom_histogram(bins=15, fill="steelblue", colour="white")
    + labs(title="Distribution of bat ages", x="Age (years)", y="Count")
    + theme_minimal()
)

# ggsave("outputs/age_distribution.png", width=7, height=5)
```
...
(
    bats_raw
    .drop(columns=["sample", "age_category"])
    .melt(id_vars="age", var_name="site", value_name="methylation")
    .pipe(lambda d: (
        ggplot(d, aes("methylation"))
        + geom_histogram(bins=20, fill="steelblue", colour="white")
        + facet_wrap("site", scales="free_y")
        + labs(title="Methylation distributions by CpG site", x="Methylation value", y="Count")
        + theme_minimal()
    ))
)

# ggsave("outputs/cpg_distributions.png", width=10, height=6)
```
...
# Save the cleaned dataset for use in scripts/02_model.R
# This means 02_model.R does not need to repeat the cleaning steps
bats_raw.to_csv("data/bat_methylation_clean.csv", index=False)
```
...
# TRANSLATION-NOTE: OQ-061 - GGally::ggpairs() has no Python equivalent, see OPEN-QUESTIONS.md
...
# Load the cleaned dataset produced by 01_explore.R
# Run that script first if this file does not exist
bats = pd.read_csv("data/bat_methylation_clean.csv").drop(columns=["sample", "age_category"])
```
...
```{python}
#| eval: false
# TRANSLATION-NOTE: OQ-058
# Fit a linear model the familiar way
# This is the inferential framing you have used before in statistics
formula = "age ~ " + " + ".join(c for c in bats.columns if c != "age")
lm_base = smf.ols(formula, data=bats).fit()
lm_base.summary()
```
...
# it tells us whether the model's assumptions are met,
# not whether it will generalise to new bats
sm.qqplot(lm_base.resid, line="45")
plt.scatter(lm_base.fittedvalues, lm_base.resid)
```
...
(
    ggplot(split_diagram)
    + geom_rect(
        aes(xmin="x", xmax="xend", ymin=0, ymax=1, fill="fill"),
        alpha=0.85, colour="white", size=1,
    )
    + geom_text(
        aes(x="(x + xend) / 2", y=0.5, label="label"),
        colour="white", fontweight="bold", size=4.5, lineheight=1.3,
    )
    + scale_fill_identity()
    + annotate("text", x=0.4, y=-0.25, label="Model is fitted on this data only", colour="#457B9D", size=3.5)
    + annotate("text", x=0.9, y=-0.25, label="Model is never seen this data until evaluation", colour="#E63946", size=3.5)
    + annotate("segment", x=0.4, xend=0.4, y=-0.15, yend=0, colour="#457B9D", arrow=arrow(length=0.2))
    + annotate("segment", x=0.9, xend=0.9, y=-0.15, yend=0, colour="#E63946", arrow=arrow(length=0.2))
    + scale_x_continuous(limits=(0, 1))
    + scale_y_continuous(limits=(-0.4, 1.2))
    + labs(title="The train/test split", subtitle="Data is divided before any modelling begins")
    + theme_void(base_size=13)
    + theme(
        plot_title=element_text(face="bold", ha="center"),
        plot_subtitle=element_text(ha="center", colour="grey40"),
    )
)
```
...
#| eval: false
# Split 1: baseline — 80/20, seed 42
train_1, test_1 = train_test_split(bats, train_size=0.8, random_state=42)

fit_1 = Pipeline([
    ("normalize", StandardScaler()),
    ("model", LinearRegression()),
]).fit(train_1.drop(columns="age"), train_1["age"])

rmse_1 = mean_squared_error(test_1["age"], fit_1.predict(test_1.drop(columns="age")), squared=False)
...
#| eval: false
# Split 2: 70/30, seed 42
# Only the train_size argument changes relative to Split 1
train_2, test_2 = train_test_split(bats, train_size=0.7, random_state=42)

fit_2 = Pipeline([
    ("normalize", StandardScaler()),
    ("model", LinearRegression()),
]).fit(train_2.drop(columns="age"), train_2["age"])

rmse_2 = mean_squared_error(test_2["age"], fit_2.predict(test_2.drop(columns="age")), squared=False)
...
#| eval: false
# Split 3: 80/20, seed 123
# Only the random_state changes relative to Split 1
# The proportion is identical — only the random assignment differs
train_3, test_3 = train_test_split(bats, train_size=0.8, random_state=123)

fit_3 = Pipeline([
    ("normalize", StandardScaler()),
    ("model", LinearRegression()),
]).fit(train_3.drop(columns="age"), train_3["age"])

rmse_3 = mean_squared_error(test_3["age"], fit_3.predict(test_3.drop(columns="age")), squared=False)
...
#| eval: false
pd.DataFrame({
    "split": ["80/20, seed 42", "70/30, seed 42", "80/20, seed 123"],
...
#| eval: false
# Commit to the 80/20 split with seed 42 for the main model
bat_train, bat_test = train_test_split(bats, train_size=0.8, random_state=42)

# TRANSLATION-NOTE: OQ-058
# step_naomit() drops rows with missing values before fitting; Pipeline has
# no row-dropping step, so this happens on the training frame directly
bat_train = bat_train.dropna()

# Model specification: ordinary linear regression, normalised predictors
# StandardScaler rescales all predictors to mean 0, standard deviation 1
lm_workflow = Pipeline([
    ("normalize", StandardScaler()),
    ("model", LinearRegression()),
])

# Fit on training data only — the test set is not touched here
lm_fit = lm_workflow.fit(bat_train.drop(columns="age"), bat_train["age"])
```
...
#| eval: false
# TRANSLATION-NOTE: OQ-058
# Pull the fitted estimator out of the pipeline and compute residuals
# manually — scikit-learn has no built-in diagnostic panel like check_model()
# This runs on the training data only
lm_train_fitted = lm_fit.predict(bat_train.drop(columns="age"))
lm_train_resid = bat_train["age"] - lm_train_fitted

sm.qqplot(lm_train_resid, line="45")
plt.scatter(lm_train_fitted, lm_train_resid)
```
...
#| eval: false
# Generate predictions on the held-out test set
lm_preds = bat_test.assign(pred=lm_fit.predict(bat_test.drop(columns="age")))[["age", "pred"]]
...
lm_metrics = pd.DataFrame({
    "metric": ["rmse", "rsq", "mae"],
    "estimate": [
        mean_squared_error(lm_preds["age"], lm_preds["pred"], squared=False),
        r2_score(lm_preds["age"], lm_preds["pred"]),
        mean_absolute_error(lm_preds["age"], lm_preds["pred"]),
    ],
})
...
#| eval: false
(
    ggplot(lm_preds, aes("age", "pred"))
    + geom_point(alpha=0.5)
    + geom_abline(linetype="dashed", colour="red")
    + geom_smooth(method="loess", colour="blue", se=True)
    + labs(x="Actual age (years)", y="Predicted age (years)", title="Linear regression — predicted vs. actual age")
    + theme_minimal()
)
```
...
(
    lm_preds
    .assign(residual=lambda d: d["age"] - d["pred"])
    .pipe(lambda d: (
        ggplot(d, aes("pred", "residual"))
        + geom_point(alpha=0.5)
        + geom_hline(yintercept=0, linetype="dashed", colour="red")
        + geom_smooth(method="loess", colour="blue", se=True)
        + labs(x="Predicted age (years)", y="Residual (actual minus predicted)", title="Residual plot — linear regression")
        + theme_minimal()
    ))
)
```
...
# Correlation structure in the training data only
# We never use bat_test for exploratory purposes
bats_cor_matrix = bats.dropna().drop(columns="age").corr()
...
(
    ...
    .pipe(lambda d: (
        ggplot(d, aes("var1", "var2", fill="correlation"))
        + geom_tile()
        + theme_minimal()
...
```{python}
#| eval: false
# Ridge specification: Pipeline with the same normalisation as linear regression
ridge_workflow = Pipeline([
    ("normalize", StandardScaler()),
    ("model", Ridge()),
])

# Define 50 candidate lambda (alpha) values and search across 5 folds,
# scoring by RMSE (GridSearchCV maximises its score, so RMSE is negated)
lambda_grid = {"model__alpha": np.logspace(-4, 4, 50)}
...
ridge_tuned.fit(bat_train.drop(columns="age"), bat_train["age"])
```
...
p <- autoplot(ridge_tuned)
  
p + scale_x_log10(labels = scales::label_log()) # force better x axis labels
```
...
tuning_results = pd.DataFrame({
    "penalty": ridge_tuned.cv_results_["param_model__alpha"],
    "rmse": -ridge_tuned.cv_results_["mean_test_score"],
})

(
    ggplot(tuning_results, aes("penalty", "rmse"))
    + geom_line()
    + geom_point()
    + scale_x_log10()
)
...
```{python}
#| eval: false
best_lambda = ridge_tuned.best_params_
...
#| eval: false
# TRANSLATION-NOTE: OQ-058
# GridSearchCV already refits the best combination — no separate
# finalize/fit step needed, .best_estimator_ is the fitted pipeline
ridge_final = ridge_tuned.best_estimator_

ridge_preds = bat_test.assign(pred=ridge_final.predict(bat_test.drop(columns="age")))[["age", "pred"]]

ridge_metrics = pd.DataFrame({
    "metric": ["rmse", "rsq", "mae"],
    "estimate": [
        mean_squared_error(ridge_preds["age"], ridge_preds["pred"], squared=False),
        r2_score(ridge_preds["age"], ridge_preds["pred"]),
        mean_absolute_error(ridge_preds["age"], ridge_preds["pred"]),
    ],
})
...
#| eval: false
(
    ggplot(ridge_preds, aes("age", "pred"))
    + geom_point(alpha=0.5)
    + geom_abline(linetype="dashed", colour="red")
    + geom_smooth(method="loess", colour="blue", se=True)
    + labs(x="Actual age (years)", y="Predicted age (years)", title="Ridge regression — predicted vs. actual age")
    + theme_minimal()
)
```
...
(
    ...
    .pipe(lambda d: (
        ...
        + geom_hline(yintercept=0, linetype="dashed", colour="red")
        + geom_smooth(method="loess", colour="blue", se=True)
        + labs(x="Predicted age (years)", y="Residual (actual minus predicted)", title="Residual plot — ridge regression")
        + theme_minimal()
    ))
)
```
...
#| eval: false
df = pd.DataFrame({
    "predictor": pd.Categorical(
        ["CpG site A\n(strong signal)", "CpG site B\n(weak signal)", "Hypothetical\nnon-predictor"],
        categories=["CpG site A\n(strong signal)", "CpG site B\n(weak signal)", "Hypothetical\nnon-predictor"],
    ),
    "lm_est": [4.2, 1.8, 3.1],
    "ridge_est": [3.8, 0.9, 0.4],
    "type": ["CpG site", "CpG site", "Synthetic noise"],
    "x_pos": [1, 2, 3],
})

(
    ggplot(df)
    + geom_col(aes(x="predictor", y="ridge_est", fill="type"), alpha=0.85, width=0.55)
    + geom_rect(
        aes(xmin="x_pos - 0.275", xmax="x_pos + 0.275", ymin="ridge_est", ymax="lm_est", colour="type"),
        fill=None, linetype="dashed", size=0.7,
    )
    + annotate("text", x=0.72, y=1.9, label="Ridge\ncoefficient", colour="white", size=2.8, fontweight="bold", lineheight=1.1)
    + annotate("segment", x=0.88, xend=0.88, y=3.82, yend=4.18, colour="#457B9D", arrow=arrow(ends="both", length=0.15))
    + scale_fill_manual(values={"CpG site": "#457B9D", "Synthetic noise": "#E63946"})
    + scale_colour_manual(values={"CpG site": "#457B9D", "Synthetic noise": "#E63946"}, guide=False)
    + scale_y_continuous(limits=(0, 5), breaks=range(0, 6))
    + labs(
        x=None,
        y="Coefficient magnitude",
        fill=None,
    ...
    + theme(
        ...
        axis_text_x=element_text(size=10, lineheight=1.2),
        plot_subtitle=element_text(colour="grey40", size=10),
...
#| eval: false
# TRANSLATION-NOTE: OQ-058
def tidy_coefs(pipeline, feature_names):
    model = pipeline.named_steps["model"]
    return pd.DataFrame({
        "term": ["Intercept"] + list(feature_names),
        "estimate": [model.intercept_] + list(model.coef_),
    })

feature_names = bat_train.drop(columns="age").columns
ridge_coefficients = tidy_coefs(ridge_final, feature_names)
lm_coefficients = tidy_coefs(lm_fit, feature_names)
```
...
(
    ridge_coefficients
    .loc[lambda d: d["term"] != "Intercept"]
    .sort_values("estimate")
    .pipe(lambda d: d.assign(term=pd.Categorical(d["term"], categories=d["term"], ordered=True)))
    .pipe(lambda d: (
        ggplot(d, aes(x="estimate", y="term"))
        + geom_col(fill="steelblue")
        + geom_vline(xintercept=0, linetype="dashed", colour="red")
        + labs(x="Coefficient estimate", y="Predictor", title="Ridge regression coefficients", subtitle="Sites close to zero have been shrunk by the penalty")
        + theme_minimal()
    ))
)
```
...
(
    pd.concat([
        lm_metrics.assign(model="Linear regression"),
        ridge_metrics.assign(model="Ridge regression"),
    ])
    .loc[lambda d: d["metric"] == "rmse"]
    [["model", "estimate"]]
    .sort_values("estimate")
)
```
...
(
    pd.concat([
        lm_preds.assign(model="Linear regression"),
        ridge_preds.assign(model="Ridge regression"),
    ])
    .pipe(lambda d: (
...
(
    pd.concat([
        lm_preds.assign(model="Linear regression", residual=lambda d: d["age"] - d["pred"]),
        ridge_preds.assign(model="Ridge regression", residual=lambda d: d["age"] - d["pred"]),
    ])
    .pipe(lambda d: (
        ggplot(d, aes("pred", "residual"))
        + geom_point(alpha=0.5)
        ...
        + geom_smooth(method="loess", colour="blue", se=True)
        + facet_wrap("model")
        + labs(x="Predicted age (years)", y="Residual")
        + theme_minimal()
    ))
)
```
...
#| eval: false
boxes = pd.DataFrame({
    "x": [0.25, 0.25, 0.25, 0.25, 0.75, 0.75, 0.75, 0.75, 0.75],
    "y": [0.90, 0.68, 0.46, 0.24, 0.90, 0.72, 0.54, 0.36, 0.18],
    "label": [
        "All data",
        "Fit model\non all data",
        "summary()",
    ...
    "col": ["#457B9D"] * 4 + ["#E63946"] * 5,
    "frame": ["Inference"] * 4 + ["Prediction"] * 5,
})

arrows_inf = pd.DataFrame({
    "x": 0.25, "xend": 0.25,
    "y": [0.86, 0.64, 0.42],
    "yend": [0.72, 0.50, 0.30],
})

arrows_pred = pd.DataFrame({
    "x": 0.75, "xend": 0.75,
    "y": [0.86, 0.68, 0.50, 0.32],
    "yend": [0.76, 0.58, 0.40, 0.22],
})

(
    ggplot()
    + annotate("text", x=0.25, y=1.0, label="INFERENCE", fontweight="bold", colour="#457B9D", size=4.5)
    + annotate("text", x=0.75, y=1.0, label="PREDICTION", fontweight="bold", colour="#E63946", size=4.5)
    + geom_label(
        boxes, aes(x="x", y="y", label="label", fill="col"),
        colour="white", fontweight="bold", size=3, lineheight=1.1, label_size=0,
    )
    + geom_segment(
        arrows_inf, aes(x="x", xend="xend", y="y", yend="yend"),
        arrow=arrow(length=0.2, type="closed"), colour="#457B9D", size=0.8,
    )
    + geom_segment(
        arrows_pred, aes(x="x", xend="xend", y="y", yend="yend"),
        arrow=arrow(length=0.2, type="closed"), colour="#E63946", size=0.8,
    )
    + scale_fill_identity()
    + scale_x_continuous(limits=(0, 1))
    + scale_y_continuous(limits=(0.1, 1.05))
    + labs(title="Two frameworks, same model", subtitle="The mathematics does not change — the question and evaluation do")
    + theme_void(base_size=13)
    + theme(
        plot_title=element_text(face="bold", ha="center", size=14),
        plot_subtitle=element_text(ha="center", colour="grey40", size=11),
    )
)
```
