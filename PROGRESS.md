...
| 20 | ml-logistic-regression.qmd | 29 (exact) | translated | 1 | | 2026-08-27 | extended the tidymodels→scikit-learn Stack with classification-specific idioms: `read_table()`, `predict(type="prob"/"class")`, `accuracy`/`sensitivity`/`specificity` (specificity has no dedicated sklearn function, computed as recall of the negative class), `conf_mat()` heatmap → `geom_tile()`, `roc_curve()` → manual plotnine plot (OQ-062, reuses OQ-058's coefficient-extraction idiom too) |
