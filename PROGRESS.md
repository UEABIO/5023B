...
| 19 | ml-regression.qmd | 34 (exact) | translated | 4 | | 2026-08-27 | large chapter, extended the already-settled tidymodels→scikit-learn Stack with several idioms it needed for the first time: `.` formula shorthand, `step_naomit()`'s row-dropping (no Pipeline equivalent), yardstick's combined rmse/rsq/mae, `extract_fit_engine`/`extract_fit_parsnip`, `autoplot(tune_grid)`, and the select_best/finalize/fit → GridSearchCV collapse (OQ-058); `corrplot()` → plotnine `geom_tile()` heatmap (OQ-059); one more `echo=F` diagram skip-logged (OQ-060); `GGally::ggpairs()` skip-logged twice, reusing OQ-026 (OQ-061) |
