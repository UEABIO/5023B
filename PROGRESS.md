...
| 17 | power_analysis_chapter.qmd | 11 (exact) | translated | 2 | 6150184 | 2026-08-26 | no new Stack package needed — numpy/scipy cover distributions and logit/expit; `replicate()`/`map_dfr()` idiom seeded via a helper-function + list-comprehension pattern (OQ-055); Poisson/Binomial `Single iteration`/`Power curve` tabsets nested rather than collapsed since both tabs are substantial independent teaching units, a second resolution to the same "pre-existing multi-tab" shape as OQ-053 (OQ-056) |
| 18 | AI-programming.qmd | ~24 | translated | 1 | | 2026-08-27 | whole chapter skip-logged (OQ-057): teaches GitHub Copilot/RStudio workflows, not R technique — embedded dplyr/ggplot2 snippets are glossary-covered but the surrounding prose is RStudio-UI-specific throughout, so no Python tabs added anywhere; no `.qmd` edits made |
| 19 | ml-regression.qmd | 34 (exact) | translated | 4 | a4afde7 | 2026-08-27 | large chapter, extended the already-settled tidymodels→scikit-learn Stack with several idioms it needed for the first time: `.` formula shorthand, `step_naomit()`'s row-dropping (no Pipeline equivalent), yardstick's combined rmse/rsq/mae, `extract_fit_engine`/`extract_fit_parsnip`, `autoplot(tune_grid)`, and the select_best/finalize/fit → GridSearchCV collapse (OQ-058); `corrplot()` → plotnine `geom_tile()` heatmap (OQ-059); one more `echo=F` diagram skip-logged (OQ-060); `GGally::ggpairs()` skip-logged twice, reusing OQ-026 (OQ-061) |
| 20 | ml-logistic-regression.qmd | 29 (exact) | translated | 1 | 132cbaf | 2026-08-27 | extended the tidymodels→scikit-learn Stack with classification-specific idioms: `read_table()`, `predict(type="prob"/"class")`, `accuracy`/`sensitivity`/`specificity` (specificity has no dedicated sklearn function, computed as recall of the negative class), `conf_mat()` heatmap → `geom_tile()`, `roc_curve()` → manual plotnine plot (OQ-062, reuses OQ-058's coefficient-extraction idiom too) |
| 21 | workshop_03_random_forests.qmd | 22 (exact) | translated | 2 | 38191ae | 2026-08-27 | extended the Stack with `roc_auc` scoring and a real distinction: R's `importance = "permutation"` needs `sklearn.inspection.permutation_importance`, not the impurity-based `.feature_importances_` the earlier `vip::vip()` glossary entry pointed to (OQ-063); three `echo=F` decorative diagrams (decision tree schematic, bagging diagram, stability simulation) skip-logged together in one entry (OQ-064) |
| 22 | workshop_04_pca_kmeans.qmd | 19 (exact) | translated | 3 | 053caf1 | 2026-08-27 | extended the Stack with `prep()`/`juice()` → `Pipeline.fit_transform()`, PCA variance/loadings extraction, and `kmeans()$tot.withinss` → `KMeans.inertia_` (OQ-065); `corrplot()` reused the `geom_tile()` heatmap idiom from `ml-regression.qmd` (OQ-059); `patchwork`'s side-by-side `p1 + p2` composition has no plotnine equivalent, shown as two separate plots instead (OQ-066). Last of the four tidymodels/scikit-learn ML chapters — Stack decision from the inventory pass is now fully exercised |
