...
# Correct model - adjusts for confounder
smf.ols("height ~ fertiliser + sunlight", data=plant_data).fit()
```
...
#| eval: false
# WRONG - adjusts for collider, creates bias
smf.ols("fledglings ~ feeding + predator_visits", data=bird_data).fit()

# CORRECT - leaves collider alone
smf.ols("fledglings ~ feeding", data=bird_data).fit()
```
...
#| eval: false
# Total effect - how does exercise affect heart disease?
smf.ols("heart_disease ~ exercise", data=exercise_data).fit()

# Direct effect - how does exercise affect heart disease
# BESIDES through weight loss?
smf.ols("heart_disease ~ exercise + weight_loss", data=exercise_data).fit()
```
...
exposure, outcome = "treatment", "outcome"
...
#| eval: false
# Identify colliders in your DAG
[n for n, d in my_dag.dag.in_degree() if d >= 2]
```
...
```{python}
#| eval: false
my_dag.get_all_backdoor_adjustment_sets(exposure, outcome)
```
...
```{python}
#| eval: false
fly_dag = CausalGraphicalModel(
    nodes=["type", "longevity", "sleep", "thorax", "partners"],
    edges=[
        ("type", "longevity"),
        ("sleep", "longevity"),
        ("thorax", "longevity"),
...
exposure, outcome = "type", "longevity"
...
```{python}
#| eval: false
[n for n, d in fly_dag.dag.in_degree() if d >= 2]
```
...
type → sleep ← partners
```
When sleep is a collider between partners and type, it means both partners and type independently cause sleep. 
...
```{python}
#| eval: false
fly_dag.get_all_backdoor_adjustment_sets(exposure, outcome)
```
...
```{r}
model_precise <- lm(longevity ~ type + thorax + partners, data = fly_data)
...
```{r}
model_wrong <- lm(longevity ~ type + thorax  + partners + sleep , data = fly_data)
...
model_total = smf.ols("longevity ~ type", data=fly_data).fit()
...
```{python}
#| eval: false
sm.qqplot(model_total.resid, line="45")
plt.scatter(model_total.fittedvalues, model_total.resid)
```
...
#| eval: false
# Kitchen sink model
smf.ols("outcome ~ treatment + var1 + var2 + var3 + var4 + var5", data=data).fit()
```
...
#| eval: false
# Always include baseline when outcome is measured post-treatment
smf.ols("diversity_change ~ treatment + baseline_diversity", data=data).fit()
```
...
4. **Check structures**
   ...
   ```{python}
   #| eval: false
   [n for n, d in my_dag.dag.in_degree() if d >= 2]  # Find colliders

   # TRANSLATION-NOTE: OQ-052 - ggdag_paths() has no Python equivalent, see OPEN-QUESTIONS.md
   ```
...
6. **Fit your model**
   ...
   ```{python}
   #| eval: false
   smf.ols("outcome ~ exposure + adjustments", data=data).fit()
   ```
...
farm_dag.draw()
farm_dag.get_all_backdoor_adjustment_sets(exposure, outcome)
```
...
#| eval: false
smf.ols(
    "exam_score ~ teaching_method + motivation + hours_studied + prior_achievement", data=exam_data
).fit()
```
...
```{python}
#| eval: false
smf.ols("exam_score ~ teaching_method + prior_achievement", data=exam_data).fit()
```
