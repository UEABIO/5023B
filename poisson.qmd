# Poisson regression (for count data or rate data) {#sec-poisson}
...
#| eval: false
(
    ggplot(cuckoo, aes("Mass", "Beg", colour="Species"))
    + geom_point(size=3, alpha=0.6)
    + scale_colour_manual(values=["darkorange", "steelblue"])
    + labs(x="Nestling mass (g)", y="Begging calls per 6 seconds")
    + theme_minimal()
)
```
...
```{python}
#| eval: false
cuckoo_lm = smf.ols("Beg ~ Mass * Species", data=cuckoo).fit()
...
```{python}
#| eval: false
# TRANSLATION-NOTE: OQ-031
sm.qqplot(cuckoo_lm.resid, line="45")
plt.scatter(cuckoo_lm.fittedvalues, cuckoo_lm.resid)
```
...
```{python}
#| eval: false
# TRANSLATION-NOTE: OQ-028
# Generate predictions
grid = pd.DataFrame(
    [(mass, species) for mass in np.arange(0, 41, 5) for species in cuckoo["Species"].unique()],
    columns=["Mass", "Species"],
)
predictions_lm = grid.assign(**cuckoo_lm.get_prediction(grid).summary_frame())

# Check for negative predictions
predictions_lm.loc[lambda d: d["mean"] < 0]
```
...
```{python}
#| eval: false
cuckoo_glm_add = smf.glm(
    "Beg ~ Mass + Species", data=cuckoo, family=sm.families.Poisson()
).fit()

cuckoo_glm_add.summary()
```
...
predictions_poisson = grid.assign(**cuckoo_glm_add.get_prediction(grid).summary_frame())

(
    ggplot(predictions_poisson, aes("Mass", "mean", colour="Species"))
    + geom_line(size=1)
    + geom_point(data=cuckoo, mapping=aes(y="Beg"), alpha=0.5)
    + scale_colour_manual(values=["darkorange", "steelblue"])
    + labs(
        x="Nestling mass (g)",
        y="Begging calls per 6 seconds",
...
```{python}
#| eval: false
sm.qqplot(cuckoo_glm_add.resid_deviance, line="45")
plt.scatter(cuckoo_glm_add.fittedvalues, cuckoo_glm_add.resid_deviance)
```
...
```{python}
#| eval: false
cuckoo_glm_add.pearson_chi2 / cuckoo_glm_add.df_resid
```
...
```{python}
#| eval: false
dispersion_add = cuckoo_glm_add.deviance / cuckoo_glm_add.df_resid
dispersion_add

# Pearson-based dispersion (performance::check_overdispersion() equivalent)
cuckoo_glm_add.pearson_chi2 / cuckoo_glm_add.df_resid
```

:::::

**Discussion:** The dispersion ratio is [~7-9]. What might cause variance to exceed the Poisson assumption?
...
```{python}
#| eval: false
cuckoo_glm_int = smf.glm(
    "Beg ~ Mass * Species", data=cuckoo, family=sm.families.Poisson()
).fit()

cuckoo_glm_int.summary()
```
...
dispersion_comparison = pd.DataFrame({
    "Model": ["Additive (no interaction)", "Interaction included"],
    "Dispersion": [
        cuckoo_glm_add.deviance / cuckoo_glm_add.df_resid,
        cuckoo_glm_int.deviance / cuckoo_glm_int.df_resid,
    ],
})
...
```{python}
#| eval: false
cuckoo_glm_int.pearson_chi2 / cuckoo_glm_int.df_resid
```
...
```{python}
#| eval: false
# TRANSLATION-NOTE: OQ-030
lr_stat = 2 * (cuckoo_glm_int.llf - cuckoo_glm_add.llf)
df_diff = cuckoo_glm_add.df_resid - cuckoo_glm_int.df_resid
...
#| eval: false
grid = pd.DataFrame(
    [(mass, species) for mass in np.arange(0, 41, 1) for species in cuckoo["Species"].unique()],
    columns=["Mass", "Species"],
)

# Generate predictions for both models
pred_additive = grid.assign(**cuckoo_glm_add.get_prediction(grid).summary_frame()).assign(Model="Additive")
pred_interaction = grid.assign(**cuckoo_glm_int.get_prediction(grid).summary_frame()).assign(Model="Interaction")

predictions_combined = pd.concat([pred_additive, pred_interaction])

(
    ggplot(predictions_combined, aes("Mass", "mean", colour="Species"))
    + geom_point(data=cuckoo, mapping=aes(y="Beg"), alpha=0.4)
...
```{python}
#| eval: false
sm.qqplot(cuckoo_glm_int.resid_deviance, line="45")
plt.scatter(cuckoo_glm_int.fittedvalues, cuckoo_glm_int.resid_deviance)
```
...
```{python}
#| eval: false
# TRANSLATION-NOTE: OQ-027
cuckoo_quasi = smf.glm(
    "Beg ~ Mass * Species", data=cuckoo, family=sm.families.Poisson()
).fit(scale="X2")

cuckoo_quasi.summary()
```
...
#| eval: false
# TRANSLATION-NOTE: OQ-027
cuckoo_negbin = smf.negativebinomial("Beg ~ Mass * Species", data=cuckoo).fit()

cuckoo_negbin.summary()
```
...
```{python}
#| eval: false
cuckoo_glm_int.aic, cuckoo_negbin.aic
```
...
```{python}
#| eval: false
sm.qqplot(cuckoo_negbin.resid, line="45")
plt.scatter(cuckoo_negbin.fittedvalues, cuckoo_negbin.resid)
```
...
coef_poisson = tidy(cuckoo_glm_int).assign(Model="Poisson")
coef_quasi = tidy(cuckoo_quasi).assign(Model="Quasi-Poisson")
coef_negbin = tidy(cuckoo_negbin).assign(Model="Negative Binomial")

coef_comparison = (
    pd.concat([coef_poisson, coef_quasi, coef_negbin])
    .loc[lambda d: d["term"] == "Mass:SpeciesWarbler"]
    [["Model", "estimate", "std_error", "conf_low", "conf_high", "p_value"]]
)
...
#| eval: false
(
    ggplot(coef_comparison, aes("Model", "estimate"))
    + geom_point(size=4)
    + geom_errorbar(aes(ymin="conf_low", ymax="conf_high"), width=0.2, size=1.2)
    + geom_hline(yintercept=0, linetype="dashed", colour="red", size=1)
    + labs(
        y="Interaction coefficient (Mass:SpeciesWarbler)",
        x="",
        title="Same data, different inferences",
    )
    + theme_minimal(base_size=14)
...
(
    coef_comparison
    .assign(
        Significant=lambda d: np.where(d["p_value"] < 0.05, "Yes", "No"),
        CI_crosses_zero=lambda d: np.where((d["conf_low"] < 0) & (d["conf_high"] > 0), "Yes", "No"),
    )
    [["Model", "estimate", "p_value", "Significant", "CI_crosses_zero"]]
)
```
...
#| eval: false
grid = pd.DataFrame(
    [(mass, species) for mass in np.arange(0, 40.5, 0.5) for species in cuckoo["Species"].unique()],
    columns=["Mass", "Species"],
)

# Final model predictions
predictions_final = grid.assign(**cuckoo_quasi.get_prediction(grid).summary_frame())

fig_main = (
    ggplot(predictions_final, aes("Mass", "mean", colour="Species", fill="Species"))
    + geom_ribbon(aes(ymin="mean_ci_lower", ymax="mean_ci_upper"), alpha=0.15, colour=None)
    # Mean estimate
    + geom_line(size=1.2)
    # Raw data
    + geom_point(data=cuckoo, mapping=aes(y="Beg"), size=2.5, alpha=0.6)
    + scale_colour_manual(
        values={"Cuckoo": "darkorange", "Warbler": "steelblue"},
        labels=["Cuckoo", "Reed warbler"],
    )
    + scale_fill_manual(
        values={"Cuckoo": "darkorange", "Warbler": "steelblue"},
        labels=["Cuckoo", "Reed warbler"],
    )
    + scale_x_continuous(breaks=np.arange(0, 41, 10))
    + scale_y_continuous(breaks=np.arange(0, 101, 10))
    + labs(
        x="Nestling mass (g)",
        y="Begging calls per 6 seconds",
        colour=None,
...
def tidy_exp(model):
    t = tidy(model)
    return t.assign(
        estimate=np.exp(t["estimate"]),
        conf_low=np.exp(t["conf_low"]),
        conf_high=np.exp(t["conf_high"]),
    )

tidy_exp(cuckoo_quasi)

# Additive model
cuckoo_quasi_add = smf.glm(
    "Beg ~ Mass + Species", data=cuckoo, family=sm.families.Poisson()
).fit()

tidy_exp(cuckoo_quasi_add)

# Predictions at specific mass values
grid_key = pd.DataFrame(
    [(species, mass) for species in cuckoo["Species"].unique() for mass in [10, 40]],
    columns=["Species", "Mass"],
)
pred_key_masses = grid_key.assign(**cuckoo_quasi.get_prediction(grid_key).summary_frame())
...
# Formal test of interaction
# TRANSLATION-NOTE: OQ-030
# No statsmodels equivalent for drop1()'s per-term F-tests; the closest
# available approach is the manual likelihood-ratio test shown earlier
# (see "Comparing model predictions"), refit per term of interest.
```
...
(
    ...
    .agg(n=("Beg", "size"), n_zeros=("Beg", lambda x: (x == 0).sum()))
    .assign(
        prop_zeros=lambda d: d["n_zeros"] / d["n"],
        pct_zeros=lambda d: d["prop_zeros"] * 100,
    )
)
```
