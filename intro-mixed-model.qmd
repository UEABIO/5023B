...
(
    ggplot(benzo_data, aes("benzo_um", "detox_exp"))
    + geom_point(alpha=0.6)
    + labs(x="Toxin concentration (µM)", y="Detoxification capacity")
    + geom_smooth(method="lm", se=True)
)
```
...
#| eval: false
(
    ggplot(benzo_data, aes("benzo_um", "detox_exp", colour="group"))
    + geom_point(alpha=0.6)
    + labs(x="Toxin concentration (µM)", y="Detoxification capacity")
...
#| eval: false
(
    ggplot(benzo_data, aes("group", "detox_exp", fill="group"))
    + geom_boxplot()
    + labs(x="Experimental batch", y="Detoxification capacity")
...
```{python}
#| eval: false
pooled_model = smf.ols("detox_exp ~ benzo_um", data=benzo_data).fit()
...
```{python}
#| eval: false
fixed_model = smf.ols("detox_exp ~ benzo_um + group", data=benzo_data).fit()
...
pooled = tidy(pooled_model).assign(Approach="Pooled")[["Approach", "term", "estimate", "std_error"]]
fixed = tidy(fixed_model).assign(Approach="Fixed Effect")[["Approach", "term", "estimate", "std_error"]]

mixed = (
    mixed_model.coefs
    .rename_axis("term")
    .reset_index()
    .rename(columns={"Estimate": "estimate", "SE": "std_error"})
    .assign(Approach="Mixed Model")
    [["Approach", "term", "estimate", "std_error"]]
)

(
    pd.concat([pooled, fixed, mixed])
    .loc[lambda d: d["term"].isin(["Intercept", "benzo_um"])]
)
```
...
```{python}
#| eval: false
grid = pd.DataFrame({"benzo_um": [0, 2.5, 5, 7.5, 10]})
mixed_model.predict(data=grid, use_rfx=False)
```
...
```{python}
#| eval: false
# TRANSLATION-NOTE: OQ-045
var_fixed = mixed_model.predict(data=benzo_data, use_rfx=False).var()
var_random = mixed_model.ranef_var.loc[mixed_model.ranef_var["Name"] != "Residual", "Var"].sum()
var_resid = mixed_model.ranef_var.loc[mixed_model.ranef_var["Name"] == "Residual", "Var"].sum()
...
```{python}
#| eval: false
sm.qqplot(mixed_model.residuals, line="45")
plt.scatter(mixed_model.fits, mixed_model.residuals)
```
...
#| eval: false
dolphins = (
    pd.read_csv("files/dolphins.csv")
    .assign(direction=lambda d: d["direction"].astype("category"))
    .dropna()
)
```
...
#| eval: false
grid = pd.DataFrame({
    "direction": dolphins["direction"].cat.categories,
    "bodymass": dolphins["bodymass"].mean(),
})
dolphmod.predict(data=grid, use_rfx=False)
```
...
rats.groupby(["Rat", "Treatment", "Liver"], as_index=False)["Glycogen"].mean().head(12)
```
...
```{python}
#| eval: false
egg_hatch_data = egg_hatch_data.assign(
    unhatched_eggs=lambda d: d["number_of_eggs"] - d["number_of_larvae"]
)

binomial_model = smf.glm(
    "number_of_larvae + unhatched_eggs ~ cross", data=egg_hatch_data, family=sm.families.Binomial()
).fit()
...
```{python}
#| eval: false
binomial_model.pearson_chi2 / binomial_model.df_resid
```
...
```{python}
#| eval: false
sm.qqplot(binomial_model.resid_deviance, line="45")
...
```{python}
#| eval: false
# TRANSLATION-NOTE: OQ-044
# Random intercept for replicate only
binomial_mixed_rep = Lmer(
    "number_of_larvae + unhatched_eggs ~ cross + (1 | replicate)", data=egg_hatch_data, family="binomial"
)
binomial_mixed_rep.fit()

# Random intercepts for replicate AND female (nested)
binomial_mixed_rep_fem = Lmer(
    "number_of_larvae + unhatched_eggs ~ cross + (1 | replicate/female_num)", data=egg_hatch_data, family="binomial"
)
binomial_mixed_rep_fem.fit()
```
...
```{python}
#| eval: false
binomial_model.aic, binomial_mixed_rep.AIC, binomial_mixed_rep_fem.AIC
```
...
```{python}
#| eval: false
(binomial_mixed_rep_fem.residuals ** 2).sum() / (len(egg_hatch_data) - len(binomial_mixed_rep_fem.coefs))
```
...
```{python}
#| eval: false
binomial_mixed_rep_fem.summary()
```
...
```{python}
#| eval: false
data = data.assign(predictor_scaled=lambda d: (d["predictor"] - d["predictor"].mean()) / d["predictor"].std())
...
```{python}
#| eval: false
# TRANSLATION-NOTE: OQ-043 - pymer4 forwards fitting options to lme4's
# lmerControl via keyword arguments on .fit(); the exact argument name
# is unconfirmed here, shown as the closest documented equivalent.
model = Lmer("response ~ predictor + (1 | group)", data=data)
model.fit(control="optimizer='bobyqa'")
```
