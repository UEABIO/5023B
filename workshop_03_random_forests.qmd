...
from sklearn.metrics import (
    accuracy_score, recall_score, confusion_matrix, roc_curve, roc_auc_score
)

# Data
dogs = pd.read_csv("data/dogs_imputed_reduced.tsv", sep=r"\s+")
dogs.columns = dogs.columns.str.lower()
...
logistic_fit = Pipeline([
    ("normalize", StandardScaler()),
    ("model", LogisticRegression()),
]).fit(dog_train.drop(columns="phenotype"), dog_train["phenotype"])
...
print("Setup complete — dogs_model, dog_train, dog_test, and logistic_preds are ready")
```
...
```{python}
#| eval: false
# Random forests do not require normalisation — they are scale-invariant
# We still dropna() as a precaution for any missing genotype values
dog_train = dog_train.dropna()

# TRANSLATION-NOTE: OQ-063
# mtry = max_features; trees = n_estimators; min_n = min_samples_split
# importance="permutation" is applied later via permutation_importance()
...
```{python}
#| eval: false
# Search grid for mtry (max_features), scored by ROC-AUC across 5 folds
rf_grid = {"model__max_features": np.linspace(2, 40, 15, dtype=int)}
...
rf_tuned.fit(dog_train.drop(columns="phenotype"), dog_train["phenotype"])
```
...
tuning_results = pd.DataFrame({
    "mtry": rf_tuned.cv_results_["param_model__max_features"],
    "roc_auc": rf_tuned.cv_results_["mean_test_score"],
})

(
    ggplot(tuning_results, aes("mtry", "roc_auc"))
    + geom_line()
    + geom_point()
)
```
...
```{python}
#| eval: false
best_mtry = rf_tuned.best_params_
...
```{python}
#| eval: false
# GridSearchCV already refits the best combination on the full training set
...
rf_preds = dog_test[["phenotype"]].assign(
    pred_affected=pred_probs[:, list(rf_final.classes_).index("affected")],
    pred_unaffected=pred_probs[:, list(rf_final.classes_).index("unaffected")],
    pred_class=rf_final.predict(X_test),
)
...
```{python}
#| eval: false
confusion_matrix(rf_preds["phenotype"], rf_preds["pred_class"], labels=["affected", "unaffected"])
```
...
```{python}
#| eval: false
cm = confusion_matrix(rf_preds["phenotype"], rf_preds["pred_class"], labels=["affected", "unaffected"])
cm_df = (
...
pd.DataFrame({
    ...
    "estimate": [
        recall_score(rf_preds["phenotype"], rf_preds["pred_class"], pos_label="affected"),
        recall_score(rf_preds["phenotype"], rf_preds["pred_class"], pos_label="unaffected"),
        accuracy_score(rf_preds["phenotype"], rf_preds["pred_class"]),
    ],
})
```
...
```{python}
#| eval: false
fpr, tpr, _ = roc_curve(rf_preds["phenotype"] == "affected", rf_preds["pred_affected"])
...
```{python}
#| eval: false
fpr, tpr, thresholds = roc_curve(rf_preds["phenotype"] == "affected", rf_preds["pred_affected"])
...
# Youden index — maximises sensitivity + specificity jointly
roc_points.assign(j_index=lambda d: d["sensitivity"] + d["specificity"] - 1).nlargest(1, "j_index")
```
...
rf_preds = rf_preds.assign(
    pred_class_custom=lambda d: np.where(d["pred_affected"] > chosen_threshold, "affected", "unaffected")
)

confusion_matrix(rf_preds["phenotype"], rf_preds["pred_class_custom"], labels=["affected", "unaffected"])
```
...
# Predicted probability distributions by true class
(
    ggplot(rf_preds, aes("pred_affected", fill="phenotype"))
    + geom_histogram(bins=20, alpha=0.7, position="identity")
...
importance = permutation_importance(
    rf_final, dog_test.drop(columns="phenotype"), dog_test["phenotype"], n_repeats=10, random_state=42
)

importance_df = (
    pd.DataFrame({
        "term": dog_test.drop(columns="phenotype").columns,
...
(
    ggplot(importance_df, aes(x="importance", y="term"))
...
# Check logistic_preds exists before comparing
# If it does not, run the quick setup code above
if "logistic_preds" not in dir():
    raise NameError("logistic_preds not found — run the quick setup code at the top of this script")

# Numeric comparison of AUC
pd.DataFrame({
    "model": ["Random forest", "Logistic regression"],
    "estimate": [
        roc_auc_score(rf_preds["phenotype"] == "affected", rf_preds["pred_affected"]),
        roc_auc_score(logistic_preds["phenotype"] == "affected", logistic_preds["pred_affected"]),
    ],
...
#| eval: false
# ROC curves for both models on the same axes
rf_fpr, rf_tpr, _ = roc_curve(rf_preds["phenotype"] == "affected", rf_preds["pred_affected"])
log_fpr, log_tpr, _ = roc_curve(logistic_preds["phenotype"] == "affected", logistic_preds["pred_affected"])

roc_comparison = pd.concat([
    pd.DataFrame({"fpr": rf_fpr, "tpr": rf_tpr, "model": "Random forest"}),
    pd.DataFrame({"fpr": log_fpr, "tpr": log_tpr, "model": "Logistic regression"}),
])

(
    ggplot(roc_comparison, aes("fpr", "tpr", colour="model"))
    + geom_line(size=1)
    + geom_abline(linetype="dashed", colour="grey50")
    + scale_colour_manual(values={"Random forest": "#E63946", "Logistic regression": "#457B9D"})
    + labs(
        x="1 - Specificity",
        y="Sensitivity",
        colour=None,
