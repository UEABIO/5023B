...
# Count matrix: genes as rows, samples as columns
# First column "Name" contains gene identifiers (e.g. CCAP1982_LOCUS00001)
rna_raw = pd.read_csv("files/ccap_count_matrix.csv")

# Sample metadata: one row per sample
meta = pd.read_csv("files/ccap_metadata.csv")
```
...
#| eval: false
# Keep only experiment 1 rows
meta1 = meta.loc[lambda d: d["experiment"] == 1]

# Subset count matrix to experiment 1 samples only
rna1 = rna_raw[["Name"] + list(meta1["id"])]
```
...
```{python}
#| eval: false
rna1_log = rna1.assign(**{
    col: np.log1p(rna1[col]) for col in rna1.select_dtypes("number").columns
})
```
...
```{python}
#| eval: false
# TRANSLATION-NOTE: OQ-065
rna1_filtered = (
    rna1_log
    .assign(mean_expr=lambda d: d.select_dtypes("number").mean(axis=1))
    .nlargest(200, "mean_expr")
...
rna1_wide = (
    rna1_filtered
    .melt(id_vars="Name", var_name="id", value_name="log_tpm")
    .pivot(index="id", columns="Name", values="log_tpm")
    .reset_index()
)
```
...
#| eval: false
# Build the correlation matrix from the wide-format data
cor_mat = rna1_wide.drop(columns="id").iloc[:, :40].corr()

# Shorten gene names for legibility
cor_mat.columns = cor_mat.columns.str.replace("CCAP1982_", "").str.replace("LOCUS", "L")
...
(
    ...
    .pipe(lambda d: (
        ...
        + theme_minimal()
        + theme(axis_text_x=element_text(rotation=90, size=6), axis_text_y=element_text(size=6))
    ))
)
```
...
X = rna1_wide.drop(columns="id")
pc_scores = pca_pipeline.fit_transform(X)

# Extract PC scores and join metadata
pca_scores = (
    pd.DataFrame(pc_scores, columns=[f"PC{i+1:02d}" for i in range(10)])
    .assign(id=rna1_wide["id"].values)
    .merge(meta1, on="id", how="left")
)
```
...
#| eval: false
conditions = [
    pca_scores["tissue"] == "YO",
    pca_scores["tissue"] == "OO",
    (pca_scores["tissue"] == "AB") & (pca_scores["sex"] == "F"),
    (pca_scores["tissue"] == "AB") & (pca_scores["sex"] == "M"),
    pca_scores["tissue"] == "TE",
    pca_scores["tissue"] == "AG",
    pca_scores["tissue"] == "EG",
]
choices = ["Young ovary", "Old ovary", "Abdomen (F)", "Abdomen (M)", "Testis", "Accessory gland", "Egg"]

pca_scores = pca_scores.assign(
    tissue_label=np.select(conditions, choices, default=pca_scores["tissue"])
)
```
...
pca_var = pd.DataFrame({
    ...
    "value": pca_model.explained_variance_ratio_[:10] * 100,
})

pca_var = pca_var.assign(label=lambda d: d["value"].round(1).astype(str) + "%")

(
    ggplot(pca_var, aes(x="component", y="value"))
    + geom_col(fill="#457B9D", alpha=0.85, width=0.65)
    + geom_text(aes(label="label"), va="bottom", fontweight="bold", size=9)
    + labs(
...
).rename_axis("terms").reset_index()

(
    loadings
    .assign(abs_value=lambda d: d["PC1"].abs())
    .nlargest(20, "abs_value")
    .assign(
        terms=lambda d: d["terms"].str.replace("CCAP1982_", ""),
        direction=lambda d: np.where(d["PC1"] >= 0, "positive", "negative"),
    )
    .sort_values("PC1")
    .pipe(lambda d: d.assign(terms=pd.Categorical(d["terms"], categories=d["terms"], ordered=True)))
    .pipe(lambda d: (
        ggplot(d, aes(x="PC1", y="terms", colour="direction"))
        + geom_vline(xintercept=0, colour="grey60")
        + geom_segment(aes(x=0, xend="PC1", y="terms", yend="terms"), size=1.2)
        + geom_point(size=3.5)
        + scale_colour_manual(values={"positive": "#457B9D", "negative": "#E63946"}, guide=False)
        + labs(
            x="Loading on PC1",
            y=None,
            title="Top 20 gene loadings on PC1",
...
```{python}
#| eval: false
pc1_pct = round(pca_var["value"].iloc[0], 1)
pc2_pct = round(pca_var["value"].iloc[1], 1)
...
(
    ggplot(pca_scores, aes(x="PC01", y="PC02", colour="tissue_label"))
    + geom_point(size=3, alpha=0.9)
    + scale_colour_manual(values=pal_tissue, name="Tissue")
    + labs(
        x=f"PC1 ({pc1_pct}% variance)",
        y=f"PC2 ({pc2_pct}% variance)",
        title="PCA biplot — Ceratitis capitata experiment 1",
        subtitle="Tissue labels were not used to compute the PCA",
    )
    + theme_minimal(base_size=13)
...
(
    pca_scores
    .loc[lambda d: d["mating_status"].notna()]
    .pipe(lambda d: (
        ggplot(d, aes(x="PC01", y="PC02", colour="tissue_label", shape="mating_status"))
        + geom_point(size=3, alpha=0.9)
        + scale_colour_manual(values=pal_tissue, name="Tissue")
        + scale_shape_manual(
            values={"M": "o", "V": "1"},
            labels={"M": "Mated", "V": "Virgin"},
            name="Mating status",
        )
        + labs(
            x=f"PC1 ({pc1_pct}% variance)",
...
```{python}
#| eval: false
pc_subset = pca_scores[["PC01", "PC02", "PC03"]]

elbow_df = pd.DataFrame({
    "k": range(1, 9),
    "wss": [KMeans(n_clusters=k, n_init=25, random_state=99).fit(pc_subset).inertia_ for k in range(1, 9)],
})

(
    ggplot(elbow_df, aes(x="k", y="wss"))
    + geom_line(colour="#457B9D", size=1.2)
    + geom_point(colour="#457B9D", size=3.5)
...
```{python}
#| eval: false
km_fit = KMeans(n_clusters=5, n_init=25, random_state=99).fit(pca_scores[["PC01", "PC02", "PC03"]])

pca_scores = pca_scores.assign(cluster=(km_fit.labels_ + 1).astype(str))
```
...
```{python}
#| eval: false
pal_cluster = {"1": "#457B9D", "2": "#E63946", "3": "#2a9d8f", "4": "#E9C46A", "5": "#8338EC"}

p_cluster = (
    ggplot(pca_scores, aes(x="PC01", y="PC02", colour="cluster"))
    + geom_point(size=3, alpha=0.9)
    + scale_colour_manual(values=pal_cluster, name="Cluster")
    + labs(x=f"PC1 ({pc1_pct}% var)", y=f"PC2 ({pc2_pct}% var)", title="K-means (k = 5)")
    + theme_minimal(base_size=12)
)
...
p_tissue = (
    ...
    + geom_point(size=3, alpha=0.9)
    + scale_colour_manual(values=pal_tissue, name="Tissue")
    + labs(x=f"PC1 ({pc1_pct}% var)", y=f"PC2 ({pc2_pct}% var)", title="Known tissue groups")
    + theme_minimal(base_size=12)
)
...
(
    pca_scores
    .groupby(["cluster", "tissue_label"], as_index=False)
    .size()
    .pivot(index="cluster", columns="tissue_label", values="size")
    .fillna(0)
)
```
