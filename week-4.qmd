...
mayfly_raw.agg(
    total_mayfly=("samples_with_mayfly", "sum"),
    total_samples=("samples_collected", "sum"),
).assign(prop_mayfly=lambda d: d["total_mayfly"] / d["total_samples"])
```
...
mayfly_raw.agg(
    max=("samples_collected", "max"),
    min=("samples_collected", "min"),
    mean=("samples_collected", "mean"),
    n_sites=("samples_collected", "size"),
    n_samples=("samples_collected", "sum"),
)
```
...
```{python}
#| eval: false
mayfly_raw.loc[lambda d: d["samples_with_mayfly"] < 1]
```
...
```{python}
#| eval: false
mayfly = mayfly_raw.assign(
    samples_without_mayfly=lambda d: d["samples_collected"] - d["samples_with_mayfly"],
    prop_mayfly=lambda d: d["samples_with_mayfly"] / d["samples_collected"],
)
```
...
```{python}
#| eval: false
# TRANSLATION-NOTE: OQ-040
# Check for impossible values
mayfly.loc[lambda d: (d["samples_collected"] < 0) | (d["samples_with_mayfly"] < 0) | (d["do_mgl"] < 0) | (d["copper_ugl"] < 0)]

mayfly.loc[lambda d: d["samples_with_mayfly"] > d["samples_collected"]]

mayfly.select_dtypes("number").agg(["max", "min"])

mayfly.loc[lambda d: (d["do_mgl"] < 3.2) | (d["do_mgl"] > 156.8)]
...
(
    ggplot(mayfly, aes("log_copper_ugl", "prop_mayfly"))
    + geom_point(aes(size="samples_collected"))
)
```
