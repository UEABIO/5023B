...
- Additional environmental variables recorded (water temperature, pH, dissolved oxygen)
...
3. **Dose-dependent**: Both presence probability AND abundance (when present) decline with copper
4. **Confounded effects**: Temperature or dissolved oxygen, not copper directly, drive patterns
...
- `stream_name`: Name of stream system (character)
...
- `total_mayflies`: Total number of mayfly individuals across all samples (integer, 0-120+)
...
- `temperature_c`: Water temperature (°C, numeric, range: 8.3-18.7)
- `ph`: Water pH (numeric, range: 6.2-8.4)
...
- Copper concentration is right-skewed (most sites low contamination, few highly polluted)
- Dissolved oxygen varies naturally (8-10 mg/L typical for healthy streams)
- Multiple sites from same stream system → potential spatial clustering (treat as independent for this analysis)
...
3. **A Quarto document** (`analysis_report.qmd`): Professional write-up with figure(s)
...
3. **Examine response variable carefully:**
   - Calculate the proportion of samples with mayflies at each site
   - Check the range of `samples_collected` (sample size varies by site)
   - Identify sites where mayflies were never found vs always found vs sometimes found
   
4. **Create analysis variables:**
   - `mayfly_proportion`: `samples_with_mayfly / samples_collected`
   - `mayfly_present`: Binary (TRUE/FALSE) — mayfly found in at least one sample?
   - Consider log-transforming copper (right-skewed, multiplicative effects expected)
...
6. **Exploratory visualisation:**
   - Plot proportion of samples with mayflies vs copper concentration
   - Consider adding point size to show sample size per site
   - Explore relationships with temperature and dissolved oxygen
   
7. **Correlation check:**
   - Are copper, temperature, and dissolved oxygen correlated?
...
mayfly <- mayfly_raw |>
  mutate(
    # Proportion with mayflies
    mayfly_proportion = samples_with_mayfly / samples_collected,
    # Binary presence
    mayfly_present = samples_with_mayfly > 0,
    # Log-transformed copper (add small constant to handle zeros if any)
    log_copper = log(copper_ugl + 0.1)
  )
...
# [Your code here - how many sites: never, always, sometimes mayfly?]
...
- **Response variable structure**: How many sites have 0%, 100%, or intermediate mayfly proportions?
- **Copper distribution**: Is it right-skewed? (Suggests log transformation appropriate)
...
- **Visual pattern**: Does mayfly proportion decline with copper? Smoothly or threshold?
...
2. **Test model structure**
   - Should copper be log-transformed?
   - Is relationship linear on logit scale, or polynomial?
   - Compare copper vs log(copper) using AIC and residual deviance
...
6. **Extract results for reporting**
...
# [Your code here]
# glm(cbind(samples_with_mayfly, samples_collected - samples_with_mayfly) ~ copper_ugl,
...
# 2. Test copper transformation ----
# Compare linear copper vs log copper
# [Your code here]

# Which has lower AIC?
# Which has lower residual deviance?

# 3. Add potential confounders ----
...
# 7. Generate data for publication figure ----
# [Your code here]
```
...
2. **Copper transformation**: Linear or log-transformed? Justify based on:
   - AIC comparison
   - Residual deviance
   - Biological plausibility

3. **Additional predictors**: Should dissolved oxygen be included?
   - Does it improve fit?
   - Is it collinear with copper?

4. **Overdispersion**: Is it present? If yes:
   - What is the dispersion parameter?
   - How much do standard errors change with quasibinomial?
   - What might cause overdispersion?

5. **Final model**: State clearly and justify
...
- **Covariates**: Water temperature (°C), dissolved oxygen (mg/L), pH
...
2. **Model selection**: 
   - Compared linear vs log-transformed copper
...
- Check how many sites are in each response category (0%, intermediate, 100%)
- The proportion of zeros matters for model interpretation
- Look at copper distribution—is log transformation needed?
- Check copper-DO correlation—strong correlation complicates interpretation
