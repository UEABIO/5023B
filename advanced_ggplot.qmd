...
plot = (
    ...
    .dropna(subset=["sex"])
    .pipe(lambda d: (
        ggplot(d, aes("species", "culmen_length_mm", fill="species"))
...
#| eval: false
# custom theme sets defaults for font and size, but these can be changed without changing the function
def theme_custom(base_size=12, base_family="serif"):
    return theme_classic(base_size=base_size, base_family=base_family) + theme(
        # specify default settings for plot titles
        plot_title=element_text(size=base_size * 1.5, face="bold", family=base_family),
        # specify defaults for axis titles
        axis_title=element_text(size=base_size, family=base_family),
        # specify position for y axis title
        axis_title_y=element_text(angle=90, margin={"r": 10, "l": 10}),
        # specify position for x axis title
        axis_title_x=element_text(margin={"t": 10, "b": 10}),
        # set major y grid lines
        panel_grid_major_y=element_line(colour="gray", size=0.5),
        # add axis lines
        axis_line=element_line(),
        # adding a margin around the plot
        plot_margin=0.02,
        # setting the position for the legend
...
heat_data = pd.DataFrame(
    [(lake, month) for lake in [f"Lake {i}" for i in range(1, 6)] for month in month_abb],
    columns=["lake", "month"],
)

# Add simulated measurements for water quality
heat_data["ph"] = np.round(np.random.normal(7, 0.5, len(heat_data)), 2)
heat_data["dissolved_oxygen"] = np.round(np.random.normal(8, 1, len(heat_data)), 1)
heat_data["nutrients"] = np.round(np.random.uniform(0, 1, len(heat_data)), 2)

# TRANSLATION-NOTE: OQ-068
# Heatmap for pH levels by lake and month
(
    ggplot(heat_data, aes(x="month", y="lake", fill="ph"))
    + geom_tile(colour="white", size=0.7)
    + scale_fill_cmap(name="pH Level", cmap_name="viridis")
    + labs(title="Monthly pH Levels by Lake", x="Month", y="Lake")
    + theme_minimal()
)
```
...
hex_data = pd.DataFrame({
    "body_size": np.random.normal(50, 15, 1000),
    "metabolic_rate": np.random.normal(200, 50, 1000),
})
...
(
    gapminder
    .loc[lambda d: d["year"] == 2007]
    .pipe(lambda d: (
        ggplot(d, aes(x="gdpPercap", y="lifeExp", colour="continent"))
        + geom_point()
        + guides(colour=False)
        + labs(title="GDP per capita", subtitle="GDP per capita raw values")
    ))
)
```
...
(
    gapminder
    .loc[lambda d: d["year"] == 2007]
    .assign(log_gdpPercap=lambda d: np.log(d["gdpPercap"]))
    .pipe(lambda d: (
        ggplot(d, aes(x="log_gdpPercap", y="lifeExp", colour="continent"))
        + geom_point()
        + guides(colour=False)
        + labs(
            title="GDP per capita, natural log (base e)",
            subtitle="GDP per capita logged manually",
        )
    ))
)
...
(
    ...
    .pipe(lambda d: (
        ...
        + guides(colour=False)
        + labs(
            title="GDP per capita, log (base 10)",
            subtitle="GDP per capita logged with scale",
        )
        + scale_x_log10()
    ))
...
(
    ...
    .pipe(lambda d: (
        ...
        + scale_x_continuous(trans=LogNaturalTrans(), labels=dollar_format(accuracy=1))
    ))
)
```
