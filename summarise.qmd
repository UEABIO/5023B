...
source("R/my_setup.R")
penguins <- readRDS(here("files", "penguins.RDS"))
```
...
```{python}
#| eval: false
len(penguins.loc[lambda d: d["species"] == "Adelie"])
```
...
```{python}
#| eval: false
penguins.groupby("species", as_index=False).size().rename(columns={"size": "n"})
```
...
```{python}
#| eval: false
# TRANSLATION-NOTE: OQ-025
# Important to include if there are any missing values
pd.DataFrame({"r": [penguins["culmen_length_mm"].corr(penguins["culmen_depth_mm"])]})
```
...
(
    penguins
    .groupby("species")
    .apply(lambda d: pd.Series({"r": d["culmen_length_mm"].corr(d["culmen_depth_mm"])}))
    .reset_index()
)
```
...
(
    penguins
    .dropna(subset=["sex"])  # Optional remove rows where Sex is unknown
    .groupby(["species", "sex"], as_index=False)
    .agg(
...
#| eval: false
# mean function
penguins.groupby("species")[["flipper_length_mm", "culmen_length_mm", "culmen_depth_mm"]].mean()
```
...
(
    ...
    + geom_histogram()
    + facet_wrap("species", ncol=1)  # stack the plots for easier comparison
)
```
